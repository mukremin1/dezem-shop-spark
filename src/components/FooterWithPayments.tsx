...
const FooterWithPayments: React.FC = () => {
    ...
    <>
      <footer role="contentinfo" style={footerStyle}>
        ...
        <nav aria-label="footer" style={navStyle}>
          <Link to="/privacy-policy" style={linkStyle} aria-label="Gizlilik Politikası sayfasına git" data-testid="footer-privacy">
            Gizlilik Politikası
          </Link>
          ...
          <a href={`mailto:${supportEmail}`} style={linkStyle} aria-label={`E-posta ile iletişim: ${supportEmail}`} data-testid="footer-contact">
            İletişim
          </a>
        </nav>

        <div style={contactStyle} aria-label="İletişim bilgileri">
          <a href={whatsappLink} target="_blank" rel="noopener noreferrer" style={{ color: "inherit", textDecoration: "none", fontWeight: 500 }} aria-label={`WhatsApp ile iletişim ${displayedPhone}`} title={`WhatsApp: ${displayedPhone}`} data-testid="footer-phone">
            {displayedPhone}
          </a>
          <div aria-hidden="false" style={{ color: "#666" }}>
            {displayedAddress}
          </div>
          <a href={`mailto:${supportEmail}`} style={{ color: "inherit", textDecoration: "none" }} aria-label={`E-posta ile iletişim ${supportEmail}`} data-testid="footer-email">
            {supportEmail}
          </a>
        </div>
      ...
      <a href={whatsappLink} target="_blank" rel="noopener noreferrer" aria-label={`WhatsApp ile iletişim ${displayedPhone}`} title={`WhatsApp: ${displayedPhone}`} data-testid="footer-whatsapp" style={whatsappButtonStyle}>
        ...
        >
          ...
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button ref={closeButtonRef} onClick={() => setTermsOpen(false)} aria-label="Kullanım Şartları penceresini kapat" style={{ background: "transparent", border: "none", fontSize: 20, cursor: "pointer", lineHeight: 1 }}>
                ×
              </button>
            </div>
            ...
            <div style={{ marginTop: 12, lineHeight: 1.6 }}>
              ...
              <p style={{ marginTop: 0 }}>
                Bu Kullanım Şartları ("Şartlar"), {siteName} tarafından sunulan web sitesinin ve hizmetlerinin ("Hizmet") kullanımına ilişkin koşulları
...
export default FooterWithPayments;
